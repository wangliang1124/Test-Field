...
export const GetList = async (ctx, next) => {
  try {
    const where = await queryToSequelizeWhereSafely(ctx.query, ctx.models.Coupon)
    ...
    const coupons = await ctx.models.Coupon.findAndCountAll(where)
    ctx.rest(coupons)
    } catch (err) {
...
export const DelectOne = async (ctx) => {
  ...
  if (ctx.errors) throw new APIError(400, ErrorsCategory[3], ctx.errors[0])
  let coupon = await ctx.models.Coupon.findById(ctx.params.id)
